// /api/drop — Quick Drop: upload a file, note, or link (owner only). List + delete.
import { dropsStore, isAuthed, json } from './_shared.mjs';
import { randomBytes } from 'node:crypto';

export const config = { path: '/api/drop' };
const MAX = 4 * 1024 * 1024; // 4 MB per file (serverless request limit)
...
export default async (req) => {
  ...
  if (req.method === 'GET') {
    ...
    for (const b of blobs) {
      ...
      if (meta && meta.metadata) items.push({ id: b.key, ...meta.metadata });
    }
  ...
  if (req.method === 'POST') {
    ...
    if (kind === 'link') {
      const test = /^https?:\/\//i.test(content) ? content : 'https://' + content;
      try { new URL(test); } catch { return json({ error: 'That doesn’t look like a valid link.' }, 400); }
      content = test;
    }
    const id = newId();
    const meta = { kind, title: String(body.title || '').trim().slice(0, 120), preview: content.slice(0, 400), createdAt: Date.now() };
